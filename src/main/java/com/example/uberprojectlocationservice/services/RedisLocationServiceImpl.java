...
public class RedisLocationServiceImpl implements LocationService{
    ...
    public Boolean saveDriverLocation(String driverId, Double latitude, Double longitude) {
        System.out.println("savedriverlocation entered");
        GeoOperations<String,String> geops=stringRedisTemplate.opsForGeo();
        System.out.println("hiii");
        try{
            geops.add(DRIVER_GEO_OPS_KEY,
        ...
        System.out.println("savedriverlocation leaved");
        return true;
    }
    ...
    public List<DriverLocationDto> getNearByDrivers(Double latitude, Double longitude) {
        ...
        for(GeoResult<RedisGeoCommands.GeoLocation<String>> result: results){
            Point point=geoOps.position(DRIVER_GEO_OPS_KEY,
                    result.getContent().getName()).get(0);
